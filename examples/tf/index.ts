import { Img, Scene, FloorObject, Entity, Raycast, DebugRay, Cooldown, BulletObject, Angle, random, Vector, PlayableCharacter, MenuUI, Constructor, KeyInputs, KeyCode, SceneUI, ImgUI, objIs } from "../../phantom2d.js";
...
class SyringeGun extends WeapBase {
    ...
    fire() {
        if(!this.cd.ready) return;
        this.cd.consume();
        scene.add(new BulletObject({ ...bulletBounds, scene, spd: 3, rot: this.pRot() + Angle.rad(random(-5, 6)), width: 15, height: 10, x: player.x, y: player.y, color: "#960d0d" }));
    }
}
...
class Wrench extends WeapBase {
    ...
    fire() {
        const out = this.ray(this.pRot(), 100);
        if(out) if(objIs(out.obj, Building)) {
...
class Bonesaw extends Melee { constructor() { super("mc.png"); } }
...
const showPDAMenu = (b1: Function, b2: Function, b3: Function, b4: Function) => {
    if(scene.hasUI(pdaMenu)) return;
    for(let i = 1; i < 5; i++) pdaMenu.bind(keyNum(i), [b1, b2, b3, b4][i-1]);
    pdaMenu.bind("q", closePDAMenu);
    scene.addUI(pdaMenu);
}
const closePDAMenu = () => {
    if(!scene.hasUI(pdaMenu)) return;
    for(let i = 1; i < 5; i++) pdaMenu.unbind(keyNum(i));
    pdaMenu.unbind("q");
    scene.rmUI(pdaMenu);
}
...
class ConstructionPDA extends PDA {
    ...
    menu() {
        ...
        for(let i = 0; i < 4; i++) pdaMenu.addChild(new ImgUI({ scene, img: new Img("mc.png") }));
        showPDAMenu(_, _, _, _);
    }
}
...
player.bind("4", () => { if(objIs(merc, Engi)) merc.swap(merc.pda1); else if(objIs(merc, Spy)) merc.swap(merc.dk); });
player.bind("5", () => { if(objIs(merc, Engi)) merc.swap(merc.pda2); });
var merc: Merc;
const weap = new Weap();
...
const keyNum = (i: number) => String(i) as KeyCode;
const showMenu = () => {
    if(scene.hasUI(menu)) return;
    for(let i = 1; i < 10; i++) menu.bind(keyNum(i), () => select(mercList[i-1]));
    scene.addUI(menu);
}
const hideMenu = () => {
    if(!scene.hasUI(menu)) return;
    for(let i = 1; i < 10; i++) menu.unbind(keyNum(i));
    scene.rmUI(menu);
}
const mercList: Constructor<Merc>[] = [Scout, Soldier, Pyro, Demo, Heavy, Engi, Medic, Sniper, Spy];
const addMercImgsToMenu = (menu: MenuUI) => {
